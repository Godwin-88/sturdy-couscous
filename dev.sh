...
DB_SERVICES="neo4j postgres redis risk-engine"
...
if [[ "${1:-}" == "--stop" ]]; then
  ...
  log "Stopping Docker databases and risk-engine..."
...
source <(grep -v '^\s*#' "$REPO/.env" | grep -v '^\s*$' \
  | grep -vE '^(NEO4J_HOST|NEO4J_PORT|POSTGRES_HOST|POSTGRES_PORT|REDIS_HOST|REDIS_PORT)=')
set +o allexport

# ── 1. Start database containers ─────────────────────────────────────────────
  log "Starting database containers + risk-engine: ${BOLD}$DB_SERVICES${RESET}"
...
wait_healthy graphalpha-neo4j
wait_healthy graphalpha-postgres
wait_healthy graphalpha-redis

# risk-engine has no health check — give it a moment to subscribe to Redis
log "Waiting for risk-engine to initialise..."
sleep 3
ok "risk-engine started (no health check — check logs with: docker logs graphalpha-risk-engine)"
...
echo -e "  Neo4j      : ${GREEN}bolt://localhost:7687${RESET}  Browser: ${GREEN}http://localhost:7474${RESET}"
echo -e "  PostgreSQL : ${GREEN}localhost:5432${RESET}"
echo -e "  Redis      : ${GREEN}localhost:6379${RESET}"
echo -e "  Risk-Engine: ${GREEN}Docker container (graphalpha-risk-engine)${RESET}"
echo ""
